...
static bool detectLuiAddiGlobal(SDNode *Tail, unsigned &Idx, SDValue &LoADDI,
  ...
  for (Idx = 0; Idx < Tail->getNumOperands(); Idx++) {
    ...
    if (!LoADDI->isMachineOpcode() ||
        !(LoADDI->getMachineOpcode() == Ceespu::ADDI) ||
...
static bool matchLuiOffset(SDValue &OffsetLUI, int64_t &Offset) {
  if (!OffsetLUI->isMachineOpcode() ||
      !(OffsetLUI->getMachineOpcode() == Ceespu::LUI) ||
...
static bool matchAddiLuiOffset(SDValue &OffsetLoADDI, int64_t &Offset) {
  ...
  if (!OffsetLoADDI->isMachineOpcode() ||
      !(OffsetLoADDI->getMachineOpcode() == Ceespu::ADDI) ||
  ...
  if (!OffsetHiLUI->isMachineOpcode() ||
      !(OffsetHiLUI->getMachineOpcode() == Ceespu::LUI) ||
...
void CeespuDAGToDAGISel::doPeepholeGlobalAddiLuiOffset() {
  ...
  while (Position != Begin) {
    ...
    if (!Tail->isMachineOpcode() || !(Tail->getMachineOpcode() == Ceespu::ADD ||
                                      Tail->getMachineOpcode() == Ceespu::ADDI))
      continue;
    ...
    int64_t Offset;
    if (Tail->getMachineOpcode() == Ceespu::ADD) {
...
void CeespuDAGToDAGISel::doPeepholeLoadStoreADDI() {
  SelectionDAG::allnodes_iterator Position(CurDAG->getRoot().getNode());
  ++Position;
  ...
  while (Position != CurDAG->allnodes_begin()) {
    ...
    if (!Base.isMachineOpcode() || Base.getMachineOpcode() != Ceespu::ADDI)
      continue;
...
// the point BuildPairF64/SplitF64 nodes are created in CeespuISelLowering, so
// some nodes would not yet have been replaced with libcalls.
void CeespuDAGToDAGISel::doPeepholeBuildPairF64SplitF64() {
  SelectionDAG::allnodes_iterator Position(CurDAG->getRoot().getNode());
  ++Position;
  ...
  while (Position != CurDAG->allnodes_begin()) {
    ...
    if (N->use_empty() || !N->isMachineOpcode() ||
        !(N->getMachineOpcode() == Ceespu::SplitF64Pseudo))
      continue;
    ...
    if (F64Val.isMachineOpcode() &&
        F64Val.getMachineOpcode() == Ceespu::BuildPairF64Pseudo) {
      LLVM_DEBUG(
