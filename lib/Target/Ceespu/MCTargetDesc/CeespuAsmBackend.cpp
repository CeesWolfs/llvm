...
class CeespuAsmBackend : public MCAsmBackend {
 public:
  CeespuAsmBackend(support::endianness Endian) : MCAsmBackend(Endian) {}
  ~CeespuAsmBackend() override = default;
...
bool CeespuAsmBackend::writeNopData(raw_ostream &OS, uint64_t Count) const {
  ...
  for (uint64_t i = 0; i < Count; i += 4)
    support::endian::write<uint32_t>(OS, 0x00210000, Endian);
...
CeespuAsmBackend::createObjectTargetWriter() const {
  return createCeespuELFObjectWriter(0);
}
...
MCAsmBackend *llvm::createCeespuAsmBackend(const Target &T,
  ...
  return new CeespuAsmBackend(support::little);
}

MCAsmBackend *llvm::createCeespuebAsmBackend(const Target &T,
                                             const MCSubtargetInfo &STI,
                                             const MCRegisterInfo &MRI,
                                             const MCTargetOptions &) {
  return new CeespuAsmBackend(support::big);
}
